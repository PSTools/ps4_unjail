...
int32_t (*orbisAudioOutSetSystemDebugState)(OrbisAudioOutSystemDebugStateElement elem, OrbisAudioOutSystemDebugStateParam *param);

int32_t (*orbisAudioOutGetSystemState)(OrbisAudioOutSystemState *state);

int32_t (*orbisAudioOutSetMixLevelPadSpk)(int32_t handle, int32_t mixLevel);

int32_t (*orbisAudioOutGetLastOutputTime)(int32_t handle, uint64_t *outputTime);

int32_t (*orbisAudioOutOutputs)(OrbisAudioOutOutputParam *param, uint32_t num);

int32_t (*orbisAudioOutSetVolume)(int32_t handle, int32_t flag, int32_t *vol);

int32_t (*orbisAudioOutOutput)(int32_t handle, const void *ptr);

int32_t (*orbisAudioOutClose)(int32_t handle);

int32_t (*orbisAudioOutOpen)(SceUserServiceUserId userId, int32_t type, int32_t index, uint32_t len, uint32_t freq, uint32_t param);

int32_t (*orbisAudioOutInit)();

int32_t (*orbisAudioOutGetPortState)(int32_t handle, OrbisAudioOutPortState *state);


void StartAudioInit(void)
{
	int LibScePad = sceKernelLoadStartModule("/system/common/lib/libSceAudioOut.sprx", 0, NULL, 0, NULL, NULL);
	sceKernelDlsym(LibScePad, "sceAudioOutSetSystemDebugState", (void **)&orbisAudioOutSetSystemDebugState);
	sceKernelDlsym(LibScePad, "sceAudioOutGetSystemState", (void **)&orbisAudioOutGetSystemState);
	sceKernelDlsym(LibScePad, "sceAudioOutGetPortState", (void **)&orbisAudioOutGetPortState);
	sceKernelDlsym(LibScePad, "sceAudioOutSetMixLevelPadSpk", (void **)&orbisAudioOutSetMixLevelPadSpk);
	sceKernelDlsym(LibScePad, "sceAudioOutGetLastOutputTime", (void **)&orbisAudioOutGetLastOutputTime);
	sceKernelDlsym(LibScePad, "sceAudioOutOutputs", (void **)&orbisAudioOutOutputs);
	sceKernelDlsym(LibScePad,"sceAudioOutSetVolume",(void **)&orbisAudioOutSetVolume);
	sceKernelDlsym(LibScePad,"sceAudioOutOutput",(void **)&orbisAudioOutOutput);
	sceKernelDlsym(LibScePad,"sceAudioOutClose",(void **)&orbisAudioOutClose);
	sceKernelDlsym(LibScePad,"sceAudioOutOpen",(void **)&orbisAudioOutOpen);
	sceKernelDlsym(LibScePad,"sceAudioOutInit",(void **)&orbisAudioOutInit);
}
...
#define BGM_DATA1	((char *)"/data/app/audio_video/sound/wave/pad_spk.wav")
...
public:
	...
	*/
	void *soundThread(void)
	{
		...
		OrbisUserServiceLoginUserIdList loginUserList;
		OrbisUserServiceEvent event;

		OrbisAudioOutOutputParam param[OUTPUTS_NUM];
		OrbisAudioOutPortState portState;
		int16_t  old_volume[OUTPUTS_NUM] = { -1, -1, -1, -1 };
		char UserName[OUTPUTS_NUM][SCE_USER_SERVICE_MAX_USER_NAME_LENGTH + 1];
		OrbisUserServiceUserColor UserColor[OUTPUTS_NUM];
		notify("Start-SoundThread-");
		printf ( "\n---Start-SoundThread---\n" );

		/*E Check the login user the current state. */
		/*J 現状のログインユーザ状態を知る */
		/*returnCode = orbisUserServiceGetLoginUserIdList(&loginUserList);
		if(returnCode != 0) {
			goto term;
		}*/
...
*/
		...
		
		{
			...
			notify("orbisAudioOutOpen");
			int32_t s_handle = orbisAudioOutOpen(userId, 
				ORBIS_AUDIO_OUT_PORT_TYPE_PADSPK, 
				0, /* index */ 
				USER_GRAIN, 
				48000, 
			...
			/*J ログインユーザ名の取得 */
		/*	returnCode = GetUserName( m_UserId[i], &UserName[i][0], 
			...
			/*E Get user color. */
			/*J ユーザカラーの取得 */
			/*returnCode = orbisUserServiceGetUserColor( m_UserId[i], &UserColor[i] ); 
			if (returnCode != 0) { 
		...
		/*E Set volume value */
		/*J ボリューム値の設定 */
		for (int i = 0; i < ORBIS_AUDIO_OUT_CHANNEL_MAX; i ++) {
			volume[i] = ORBIS_AUDIO_VOLUME_0dB;
		}
		...
		for ( int i = 0 ; i < loginUserNumber ; i++ ) {
			...
			orbisAudioOutSetVolume( m_handle[i], 
				( ORBIS_AUDIO_VOLUME_FLAG_FL_CH | ORBIS_AUDIO_VOLUME_FLAG_FR_CH
				| ORBIS_AUDIO_VOLUME_FLAG_CNT_CH | ORBIS_AUDIO_VOLUME_FLAG_LFE_CH 
				| ORBIS_AUDIO_VOLUME_FLAG_RL_CH  | ORBIS_AUDIO_VOLUME_FLAG_RR_CH
				| ORBIS_AUDIO_VOLUME_FLAG_BL_CH  | ORBIS_AUDIO_VOLUME_FLAG_BR_CH ), volume);
		}
		...
		do
		{
			...
			/*J ログインユーザ情報の取得 */
			returnCode = orbisUserServiceGetEvent(&event);
			if ( returnCode < 0 ) {
				if (returnCode != ORBIS_USER_SERVICE_ERROR_NO_EVENT) {
				...
				if ( event.eventType == 0 ) {
					...
					/*J 空きに登録 */
					if ( VacantIndex != -1 ) {
						m_handle[VacantIndex] = orbisAudioOutOpen(m_UserId[VacantIndex], 
							ORBIS_AUDIO_OUT_PORT_TYPE_PADSPK, 
							0, /* index */ 
							USER_GRAIN, 
							48000, 
						...
						/*E Get login user name. */
						/*J ログインユーザ名の取得 */
						returnCode = GetUserName( m_UserId[VacantIndex], &UserName[VacantIndex][0], 
							SCE_USER_SERVICE_MAX_USER_NAME_LENGTH + 1 ); 
						if (returnCode != 0) { 
							printf ( "Error: sceUserServiceGetUserName() 0x%08x\n", returnCode );
							goto term;
						...
						/*E Get user color. */
						/*J ユーザカラーの取得 */
						returnCode = orbisUserServiceGetUserColor( m_UserId[VacantIndex], &UserColor[VacantIndex] ); 
						if (returnCode != 0) { 
							printf ( "Error: sceUserServiceGetUserColor() 0x%08x\n", returnCode );
							goto term;
						} 
				...
				/*J ログアウト */
					...
					for (int i = 0 ; i < OUTPUTS_NUM ; i++ ) {
						if ( event.userId == m_UserId[i] ) {
							...
							param[i].ptr = NULL;
							returnCode = orbisAudioOutOutputs(&param[0], loginUserNumber);
							if ( returnCode < 0 ) {
								printf("Error: orbisAudioOutOutputs() 0x%08x\n", returnCode);
								goto term;
							}
							//E Release output port.
							//J ポートの解放
							returnCode = orbisAudioOutClose(m_handle[i]);
							if ( returnCode < 0 ) {
								printf("Error: orbisAudioOutClose() 0x%08x\n", returnCode);
							}
							/*E Deregistration. */
							/*J ユーザの削除 */
							m_UserId[i] = 0;
			...
			for (int i = 0 ; i < OUTPUTS_NUM ; i++ ) {
				if ( m_UserId[i] != 0 ) {
					/*E Get port status. */
					/*J ポートステータスの取得 */
					returnCode = orbisAudioOutGetPortState( m_handle[i], &portState );
					if ( returnCode < 0 ) {
						printf ( "Error: orbisAudioOutGetPortState() 0x%08x\n", returnCode );
						goto term;
					}
					...
					if ( old_volume[i] != portState.volume ) {
						...
						printf("[UserName:%s(Color:%s)] pad speaker volume change to %d\n", 
							&UserName[i][0], 
							UserColor[i] == ORBIS_USER_SERVICE_USER_COLOR_BLUE  ? "BLUE"  :
							UserColor[i] == ORBIS_USER_SERVICE_USER_COLOR_RED   ? "RED"   :
							UserColor[i] == ORBIS_USER_SERVICE_USER_COLOR_GREEN ? "GREEN" :
							UserColor[i] == ORBIS_USER_SERVICE_USER_COLOR_PINK  ? "PINK"  : "Unknown",
							portState.volume );
	...
	int32_t initialize( int8_t *sound, uint32_t ch, uint32_t size )
	{
		...
		if ( m_StateFlag == AUDIO_STATUS_END ) {
			...
			if(returnCode != 0) {
				...
				char buffer[512];
				sprintf(buffer, "sceUserServiceInitialize Error:0x%08x", returnCode);//dont want all those enters
				notify(buffer);
			}
			m_StateFlag = AUDIO_STATUS_NONE;
		}
		...
		/*J AudioOut初期処理  */
		notify("Intilize orbisAudioOutInit");
		returnCode = orbisAudioOutInit();
		if ( ( returnCode != 0 ) && ( returnCode != ORBIS_AUDIO_OUT_ERROR_ALREADY_INIT ) ) {
			fprintf( stderr, "Failed orbisAudioOutInit():0x%08x\n", returnCode );
			char buffer[512];
				sprintf(buffer, "Failed orbisAudioOutInit():0x%08x\n", returnCode );//dont want all those enters
				notify(buffer);
			return 1;
		}	
		notify("Intilize Sound Thread");
...
int readWavFile(void)
{
	...
	//E WAV file open.
	//J WAVファイルオープン
	wavFileFileDescriptor = sceKernelOpen( BGM_DATA1, O_RDONLY, 0666 );
